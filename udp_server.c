...
/*This function is to encrypt/ decrypt the data*/
	...
	while(1)
    {
        ...
        else if(strcmp(receive_packet.command, "delete") == 0) //If command is delete, then server deletes particular file
        {
            if(remove(receive_packet.filename) ==  0) //If file present then delete file
            {
                ...
                strcpy(sender_packet.data, "success");
            }
            else
            {
                printf("Error: Delete Unsuccesful\n");
                strcpy(sender_packet.data, "failure");
            }
            ...
            bzero(sender_packet.data, sizeof(sender_packet.data));
...
void put_file(struct sockaddr_in remote, int remote_length, int sockfd)
{
    ...
    fd = open( receive_packet.filename, O_RDWR|O_CREAT|O_TRUNC , 0666);
    unsigned int number_of_packets = receive_packet.total_packets;
    ...
    while(1)
    {
        ...
        struct timeval timeout = {0,800000}; //Set timeout to 800ms
        ...
        {
            printf("Time out, Not received any packet in 800ms. Please check if client is still active and try again\n"); //If server is inactive for 800ms, then it stops the transfer and waits for command
            break;
        }
    ...
    if ((expected_sequence_number - 1) == number_of_packets) //Check if file transfer complete
    {
...
void get_file(struct sockaddr_in remote, int remote_length, int sockfd, fd_set readset)
{
    ...
    temp_fp = fopen(receive_packet.filename, "r");
    if (temp_fp == NULL)
    {
	...
	******************/
    ...
    if((seq_number_count - 1) == number_of_packets)
        ...
        printf("Error: Could not transfer the file successfully\n");
